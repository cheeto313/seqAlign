...
void doWork() {
	...
	while (workQueue != NULL) {
		...
		workItem = workQueue->item;

		for (int i = workItem.fromX; i < workItem.toX; i++) {
			for (int j = workItem.fromY; j < workItem.toY; j++) {
				dpMatrix[i][j] = computeSimilarity(i, j, seq1, seq2);
			}
		}

		if (workItem.x + 1 < numBlocks_x) {
			workItem_t nextItem = work[workItem.x + 1][workItem.y];
			if (queued[nextItem.x][nextItem.y] == 0) {
				queued[nextItem.x][nextItem.y] = 1;

				// Add item to the right
				workQueue_t* current = workQueue;
				while (current->next != NULL){
					current = current->next;
				}

				struct node* newWork = malloc(sizeof(struct node));
				newWork->item = nextItem;
				newWork->next = NULL;
				current->next = newWork;
			}
		}

		if (workItem.y + 1 < numBlocks_y) {
			workItem_t nextItem = work[workItem.x][workItem.y + 1];
			if (queued[nextItem.x][nextItem.y] == 0) {
				queued[nextItem.x][nextItem.y] = 1;
...
void compute(char* seq1, char* seq2) {
...
int main(int argc, char* argv[]) {
	...
	if (argc < 2) {
		fprintf(stderr, "Usage: seqAlign <data file> [output file]\n");
		exit(EXIT_FAILURE);
	}
	...
	if (argc == 3) {
		printf("Starting output (may take a while).\n");
